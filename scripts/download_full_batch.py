...
with open('data/training_batch_full.csv', newline='', encoding='utf-8') as f:
...
for i, row in enumerate(rows, 1):
    ...
    if os.path.exists(out_path):
        if is_valid_json(out_path):
            ...
            if i % 20 == 0:
                print(f'[{i}/{len(rows)}] ...already have {skipped_already_present} so far, skipping valid existing files')
            continue
        else:
            print(f'[{i}/{len(rows)}] {md5} -- existing file corrupted/truncated, re-downloading')
            os.remove(out_path)
...
print('')
print(f'Already present and valid: {skipped_already_present}')
print(f'Newly downloaded: {len(rows) - skipped_already_present - len(failed)}')
